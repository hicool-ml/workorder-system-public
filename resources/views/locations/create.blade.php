...
<div class="flex items-center justify-between mb-6 gap-3 flex-wrap">
    <div>
        ...
        <p class="text-sm text-ink-muted mt-0.5">填写地址信息</p>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2">
        <form method="POST" action="{{ route('locations.store') }}" class="card p-5 space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="label" for="name">地址名称 <span class="text-red-500">*</span></label>
                    <input type="text" class="input" id="name" name="name" value="{{ old('name') }}" required maxlength="255" placeholder="如：老校区1-7教">
                    @error('name')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="label" for="building_code">楼栋代码</label>
                    <input type="text" class="input" id="building_code" name="building_code" value="{{ old('building_code') }}" maxlength="50" placeholder="如：1-7, 8-14, A-J">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select class="input" id="building_type" name="building_type" required>
                        ...
                        <option value="{{ $key }}" {{ old('building_type') == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea class="input" id="description" name="description" rows="3" placeholder="地址的详细描述">{{ old('description') }}</textarea>
    ...
    <div class="lg:col-span-1 space-y-4">
        <div class="card p-5">
            ...
            <ul class="space-y-2 text-sm" style="color: var(--c-ink-muted);">
                <li><strong style="color: var(--c-ink);">地址名称：</strong>具体地址描述</li>
                <li><strong style="color: var(--c-ink);">楼栋代码：</strong>楼栋编号</li>
                <li><strong style="color: var(--c-ink);">校区：</strong>所属校区</li>
                <li><strong style="color: var(--c-ink);">排序：</strong>控制显示顺序</li>
            </ul>
        </div>
        <div class="card p-5">
