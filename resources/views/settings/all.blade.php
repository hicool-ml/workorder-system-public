    ...
    <div class="card max-w-md w-full">
        ...
        <form method="POST" action="{{ route('system-settings.update') }}" class="p-5 space-y-4">
            ...
            <div>
                ...
                <input type="text" class="input" id="edit_value" name="settings[edit_key]" autocomplete="off">
                <p class="text-xs mt-1" style="color: var(--c-ink-subtle);" id="edit_value_hint">根据设置类型输入相应的值</p>
            </div>
...
function editSetting(key, value, type) {
    ...
    var hint = document.getElementById('edit_value_hint');
    valueInput.name = 'settings[' + key + ']';
    valueInput.value = value;
    hint.textContent = '根据设置类型输入相应的值';
...
document.addEventListener('click', function(e) {
    ...
    if (isSecret) {
        ...
        document.getElementById('edit_value_hint').textContent = '出于安全考虑旧值已隐藏；留空提交将保留原值。';
    } else {
