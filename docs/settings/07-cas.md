...
接入单位的 CAS / LinkID 统一身份认证。启用后，登录页会出现「统一身份认证」入口，用户点击后跳转到身份认证服务方页面，认证成功后自动回跳并在本系统创建账号。CAS 账号与本地账号共存。
...
4. 配置好用户属性映射
5. 勾选启用并保存
6. 登录页出现「统一身份认证」按钮，用户点击即跳转到身份认证服务方页面

## CAS 用户行为约定

- 自动创建为本系统的**普通用户**角色（只能通过 `/report` 报修）
- CAS 用户的个人信息和密码由身份认证服务方管理，**无法在本系统内修改**（个人资料页会提示）
