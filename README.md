...
- **企业微信群通知** — 支持两种推送模式：「群机器人 Webhook」简单易用，消息仅在企业微信App内可见；「企业自建应用」可将消息直接推送到个人微信的「微信插件」，成员无需打开企业微信App即可查看完整内容。两种模式均通过系统设置配置（不硬编码），支持按事件 x 通道规则矩阵控制
...
系统支持两种企业微信推送模式，在「系统设置 → 集成配置 → 企业微信」中切换。
...
| **@指定用户** | 支持（通过 userid 或 @all） | 支持（通过 userid 或 @all） |
| **消息格式** | Markdown / Text | Text（自动转纯文本，兼容个人微信） |
| **适用场景** | 快速接入、内部通知 | 需要消息直达个人微信、正式生产环境 |

#### 群机器人模式

1. 在企业微信群聊中点击右上角 →「群机器人」→「添加机器人」
2. 复制生成的 Webhook 地址（格式：`https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx`）
3. 在系统设置中粘贴地址并启用

**局限**：群机器人消息不支持同步到微信插件（个人微信）。成员在个人微信中只能看到一个提醒，消息内容被锁在企业微信 App 内。如果需要消息直接显示在个人微信中，请使用自建应用模式。

#### 自建应用模式

1. 登录 [企业微信管理后台](https://work.weixin.qq.com/wework_admin/frame)
2. 进入「应用管理」→「自建」→ 创建应用，设置名称和可见范围
3. 获取以下凭证：
   - **CorpID**：「我的企业」→「企业信息」→「企业ID」
   - **Secret**：应用详情页 →「Secret」→「查看」（仅显示一次，忘记需重置）
   - **AgentID**：应用详情页顶部的数字 ID
4. 配置企业可信 IP：应用详情页 →「开发者接口」→「企业可信IP」，填入服务器出口公网 IP
5. 在系统设置中填入 CorpID、Secret、AgentID，切换到自建应用模式并启用

> **重要限制**：配置企业可信 IP 前，企业微信要求先设置「可信域名」或「接收消息服务器 URL」，且域名的 ICP 备案主体必须与当前企业主体相同或有关联关系。未备案域名无法通过校验。

#### SSL 证书管理

系统设置页面提供内置的 SSL 证书管理，无需手动修改 `php.ini`：

- **上传 CA 证书**：从 https://curl.se/ca/cacert.pem 下载 `cacert.pem`，在配置页直接上传，系统自动启用 HTTPS 验证
- **关闭验证**：提供开关用于测试环境，关闭时会显示安全风险提醒（中间人攻击风险，仅限测试）
