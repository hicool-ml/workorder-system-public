...
- **校区管理并入地址树**：移除独立的「校区」表与「区域管理」页面，区域/园区统一在「地址管理 → 地址树」中维护，层级结构由「层级定义」灵活配置，数据单一来源、不再双轨
- **地址层级解耦**：工单地址不再固定"校区→楼栋"两级概念，改为按日常层级的**位置**自适应（第 1 级=区域/园区、第 2 级=楼栋、第 3 级=房间/工位）。调整「层级定义」即可适配园区、门店、楼宇等不同场景，无需改动代码
- **工单地址与报表随层级联动**：工单表单的区域/楼栋下拉、列表区域筛选、报表区域统计均按日常层级位置动态解析，与「层级定义」始终保持一致

### 统计报表通用化

- 报表首页「重点分类 Top10 分布」不再固定展示特定分类，改为动态展示所有有工单数据的根分类，任意行业的分类体系均可适配
- 故障类型占比趋势图的分类排序改为跟随「分类管理」中的排序设置

### 工单创建体验

- 工单来源默认「电话报修」，应用分类模板时不再改变默认来源
- 附件支持「选择文件」与「拍照」任意组合、逐个追加，可一次提交多个附件；追加附件时已填写的附件描述自动保留
