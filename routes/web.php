...
// 测试路由（无需认证）
Route::get('/test/categories', [App\Http\Controllers\TestController::class, 'categories'])->name('test.categories');
Route::get('/test/api/subcategories', [App\Http\Controllers\TestController::class, 'getSubCategories'])->name('test.api.subcategories');

// 通知功能测试路由（需要认证）
Route::middleware(['auth'])->group(function () {
    Route::get('/test_final_notification.php', function () {
        return view('test_final_notification');
    })->name('test.final.notification');
    
    Route::get('/test_notification_frontend.php', function () {
        return file_get_contents(base_path('test_notification_frontend.php'));
    })->name('test.notification.frontend');
    
    Route::get('/test_final_notification_fixes.php', function () {
        return file_get_contents(base_path('test_final_notification_fixes.php'));
    })->name('test.final.notification.fixes');
    
    Route::get('/test_notification_center_debug', function () {
        return file_get_contents(base_path('test_notification_center_debug.php'));
    })->name('test.notification.center.debug');
    
    // URL调试页面
    Route::get('/test-url-debug', function () {
        return view('test_url_debug');
    })->name('test.url.debug');
    
    // 协议自适应修复测试页面
    Route::get('/test-protocol-fix', function () {
        return view('test_protocol_fix');
    })->name('test.protocol.fix');
    
    // 所有修复验证测试页面
    Route::get('/test-all-fixes', function () {
        return view('test_all_fixes');
    })->name('test.all.fixes');
    
    // CSS兼容性测试页面
    Route::get('/test-css-compatibility', function () {
        return view('test_css_compatibility');
    })->name('test.css.compatibility');
    
    // 字符集修复测试页面
    Route::get('/test-charset-fix', function () {
        return view('test_charset_fix');
    })->name('test.charset.fix');
    
    // CSS兼容性修复测试页面
    Route::get('/test-css-fix', function () {
        return view('test_css_fix');
    })->name('test.css.fix');
    
    // Meta元素修复测试页面
    Route::get('/test-meta-fix', function () {
        return view('test_meta_fix');
    })->name('test.meta.fix');
    
    // 附件预览测试页面
    Route::get('/test-attachment-preview', [App\Http\Controllers\TestAttachmentPreviewController::class, 'index'])->name('test.attachment.preview');
    
    // 全选框修复测试页面
    Route::get('/test-checkbox-fix', function () {
        return file_get_contents(base_path('test_checkbox_fix.php'));
    })->name('test.checkbox.fix');
    
    // 用户权限检查页面
    Route::get('/test-user-permissions', function () {
        return view('test_user_permissions');
    })->name('test.user.permissions');
    
    // 批量分配功能测试页面
    Route::get('/test-batch-assign', function () {
        return view('test_batch_assign');
    })->name('test.batch.assign');
    
    // 批量完结功能测试页面
    Route::get('/test-batch-complete', function () {
        return file_get_contents(base_path('test_batch_complete.php'));
    })->name('test.batch.complete');
    
    // 批量完结功能API测试页面
    Route::get('/test-batch-complete-api', function () {
        return file_get_contents(base_path('test_batch_complete_api.php'));
    })->name('test.batch.complete.api');
